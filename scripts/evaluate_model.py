...
def parse_args():
    ...
    group_iou_thresh.add_argument('--iou-analysis', action='store_true', default=True,
    ...
    parser.add_argument('--es-analysis', action='store_true', default=False)
...
def main():
    ...
    for dataset_name in args.datasets.split(','):
        ...
        for checkpoint_path in checkpoints_list:
            ...
            dataset_results = evaluate_dataset(dataset, predictor, args.es_analysis, pred_thr=args.thresh,
...
def save_results(args, row_name, dataset_name, logs_path, logs_prefix, dataset_results,
    ...
    all_ious, elapsed_time, all_scribbles, all_points, all_dices, all_hd95s, all_assds, all_spis = dataset_results

    mean_spc, mean_spi = utils.get_time_metrics(all_ious, elapsed_time)

    timeSum = 0
    for spi in all_spis:
        timeSum += pow(abs(spi - mean_spi), 2)
    offset_spi = (timeSum / len(all_spis)) ** 0.5

    iou_thrs = np.arange(0.8, min(0.95, args.target_iou) + 0.001, 0.05).tolist()
    noc_list, over_max_list, over_max_id_list = utils.compute_noc_metric_my(all_ious, iou_thrs=iou_thrs, max_clicks=args.n_clicks)
    ...
    header, table_row = utils.get_results_table(noc_list, over_max_list, row_name, dataset_name,
                                                mean_spc, elapsed_time, mean_spi, offset_spi, args.n_clicks,
                                                model_name=model_name)

    if args.print_ious:
        min_num_clicks = min(len(x) for x in all_ious)

        # output ratio of scribbles
        if args.es_analysis:
            mean_scribble = np.array([x[:min_num_clicks] for x in all_scribbles]).mean(axis=0)
            mscribble_str = ' '.join([f'mEs@{click_id}={mean_scribble[click_id - 1]:.0%};'
                                for click_id in range(1, 21) if click_id <= min_num_clicks])
            table_row += '\n' + mscribble_str
        
            mean_points = np.array([x[:min_num_clicks] for x in all_points]).mean(axis=0)
            offset_points = []
            for i, mean_point in enumerate(mean_points):
                pointSum = 0
                for points in all_points:
                    pointSum += pow(abs(points[i] - mean_point), 2)
                standard_deviation = (pointSum / len(all_points)) ** 0.5
                offset_points.append(standard_deviation)
            mpoint_str = ' '.join([f'mPoint±sDev@{click_id}={mean_points[click_id - 1]+click_id:.1f}±{offset_points[click_id - 1]:.1f};'
                                for click_id in range(1, 21) if click_id <= min_num_clicks])
            table_row += '\n\n' + mpoint_str

        mean_ious = np.array([x[:min_num_clicks] for x in all_ious]).mean(axis=0) # 压缩行，对各列求均值
        miou_str = ' '.join([f'mIoU@{click_id}={mean_ious[click_id - 1]:.2%};'
                             for click_id in range(1,21) if click_id <= min_num_clicks])
        table_row += '\n\n' + miou_str

        # output medical metric measures
        # dice
        mean_dices = np.array([x[:min_num_clicks] for x in all_dices]).mean(axis=0)
        offset_dices = []
        for i, mean_dice in enumerate(mean_dices):
            diceSum = 0
            for dices in all_dices:
                diceSum += pow(abs(dices[i] - mean_dice), 2)
            standard_deviation = (diceSum / len(all_dices)) ** 0.5
            offset_dices.append(standard_deviation)
        mdice_str = ' '.join([f'mDice±sDev@{click_id}={mean_dices[click_id - 1]:.2%}±{offset_dices[click_id - 1]:.2%};'
                             for click_id in range(1, 21) if click_id <= min_num_clicks])
        table_row += '\n\n' + mdice_str
        
        # hd95
        # mean_hd95s = np.array([x[:min_num_clicks] for x in all_hd95s]).mean(axis=0)
        # mhd95_str = ' '.join([f'mHd95@{click_id}={mean_hd95s[click_id - 1]:.2f};'
        #                      for click_id in range(1, 21) if click_id <= min_num_clicks])
        # table_row += '\n\n' + mhd95_str
        ...
        offset_assds = []
        for i, mean_assd in enumerate(mean_assds):
            assdSum = 0
            for assds in all_assds:
                assdSum += pow(abs(assds[i] - mean_assd), 2)
            standard_deviation = (assdSum / len(all_assds)) ** 0.5
            offset_assds.append(standard_deviation)
        massd_str = ' '.join([f'mAssd±sDev@{click_id}={mean_assds[click_id - 1]:.2f}±{offset_assds[click_id - 1]:.2f};'
                             for click_id in range(1, 21) if click_id <= min_num_clicks])
        table_row += '\n\n' + massd_str
    ...
    if log_path.exists():
        with open(log_path, 'a') as f:
            f.write(table_row + '\n')
            f.write('over_max_id_list is:\n')
            f.write(str(over_max_id_list))
    else:
...
def save_iou_analysis_data(args, dataset_name, logs_path, logs_prefix, dataset_results, model_name=None):
    all_ious, _ , _ , _ , all_dices, all_hd95s, all_assds, all_times_per_image = dataset_results
...
def get_prediction_vis_callback(logs_path, dataset_name, prob_thresh):
    save_path = logs_path / 'pred_att_vis' / dataset_name
    save_path_pred = logs_path / 'pred_vis' / dataset_name
    save_path.mkdir(parents=True, exist_ok=True)
    save_path_pred.mkdir(parents=True, exist_ok=True)

    def callback(image, gt_mask, gt_edge, pred_probs, sample_id, click_indx, clicks_list):
        sample_path = save_path / f'{sample_id}_{click_indx}.jpg'
        sample_path_pred = save_path_pred / f'{sample_id}_{click_indx}.jpg'
        # prob_map = draw_probmap(pred_probs)
        image_with_mask = draw_with_blend_and_clicks(image, pred_probs > prob_thresh, gt_edge, clicks_list=clicks_list)
        # cv2.imwrite(str(sample_path), np.concatenate((image_with_mask, prob_map), axis=1)[:, :, ::-1])
        cv2.imwrite(str(sample_path_pred), image_with_mask[:, :, ::-1])
