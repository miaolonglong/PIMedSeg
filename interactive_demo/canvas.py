...
class CanvasImage:
    ...
    def __init__(self, canvas_frame, canvas):
        ...
        self.canvas.bind('<ButtonPress-1>', self.__left_mouse_button_pressed)
        ...
        # self.canvas.bind("<B1-Motion>", self.xFunc1)
    ...
    def __left_mouse_button_pressed(self, event):
        ...
        self._last_lb_click_time = time.time()
        self._last_lb_click_event = event
        self.canvas.scan_mark(event.x, event.y)

    def __left_mouse_button_released(self, event):
        self.last_points = None
        time_delta = time.time() - self._last_lb_click_time
        move_delta = math.sqrt((event.x - self._last_lb_click_event.x) ** 2 +
                               (event.y - self._last_lb_click_event.y) ** 2)
        ...
        coords = self._get_click_coordinates(self._last_lb_click_event)
