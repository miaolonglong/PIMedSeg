...
class InteractiveController:
    def __init__(self, net, device, predictor_params, update_image_callback, prob_thresh=0.5):
        ...
        self.scribble_enable = False
        # self.es_cnt = 1
        self.interaction_round = 0
        self.cur_is_positive = False
        self.prev_edge_scribbles = None
        self.last_edge_scribbles = None

    def set_image(self, image):
        self.image = image
        self.edges = np.zeros((2,image.shape[0],image.shape[1]),dtype=np.uint8)
        self.edge_scribbles = Image.new('L', (image.shape[1], image.shape[0]))
        self.empty_mask = self.edge_scribbles.copy()
        self.edge_scribbles_draw = ImageDraw.Draw(self.edge_scribbles)
        self._result_mask = np.zeros(image.shape[:2], dtype=np.uint16)
        self.object_count = 0 
        self.reset_last_object(update_image=False)
        self.update_image_callback(reset_canvas=True)
        self.scribble_map = np.zeros((self.image.shape[0], self.image.shape[1]), dtype=np.uint8)
        self.scribble_enable = True
    ...
    def add_click(self, x, y, is_positive):
        self.interaction_round += 1
        self.cur_is_positive = is_positive
        self.states.append({
        ...
        if self.last_edge_scribbles is None:
            corrected_edge_scribble = np.array(self.edge_scribbles)
        else:
            corrected_edge_scribble = np.array(self.edge_scribbles) - self.last_edge_scribbles
            self.prev_edge_scribbles = self.last_edge_scribbles
        self.last_edge_scribbles = np.array(self.edge_scribbles).copy()

        if is_positive:
            self.edges[0] = ((self.edges[0] + corrected_edge_scribble) > 0 ).astype(np.uint8)
        else:
            self.edges[1] = ((self.edges[1] + corrected_edge_scribble) > 0 ).astype(np.uint8)

        pred = self.predictor.get_prediction(self.clicker, prev_mask=self._init_mask, edges=self.edges) #得到二维mask
        ...
        if self.probs_history:
            self.probs_history.append((self.probs_history[-1][0], pred)) #后续分割的预测，tuple（pre_mask，mask）
        else:
            self.probs_history.append((np.zeros_like(pred), pred)) #第一次分割的预测，tuple（全黑图，mask）
    ...
    def undo_click(self):
        ...
        if self.prev_edge_scribbles is not None:
            idx = 1
            if self.cur_is_positive:
                idx = 0
            self.edges[idx] = self.prev_edge_scribbles.copy().astype(np.uint8)
        self.update_image_callback()
    ...
    def get_visualization(self, alpha_blend, click_radius):
        ...
        #叠加之前预测的分割结果
        if self.probs_history:
            total_mask = self.probs_history[-1][0] > self.prob_thresh #概率图中像素值大于阈值的是mask，得到二值图
