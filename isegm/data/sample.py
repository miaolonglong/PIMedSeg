...
class DSample:
    def __init__(self, image, encoded_masks, boundarys=None, 
                
                objects=None, objects_ids=None, ignore_ids=None, sample_id=None):
        self.image = image
        self._encoded_boundary = boundarys
        ...
        self._original_data = self.image, self._encoded_masks, self._encoded_boundary, deepcopy(self._objects)

    def augment(self, augmentator):
        self.reset_augmentation()
        self.image_clone = self.image
        aug_output = augmentator(image=self.image, mask=self._encoded_masks)
        edge_output = augmentator(image=self.image_clone, mask=self._encoded_boundary)

        self.image = aug_output['image']
        self._encoded_masks = aug_output['mask']
        self._encoded_boundary = edge_output['mask']
    ...
    def reset_augmentation(self):
        if not self._augmented:
            return
        orig_image, orig_masks, orig_boundarys, orig_objects = self._original_data
        self.image = orig_image
        self._encoded_masks = orig_masks
        self._encoded_boundary = orig_boundarys
