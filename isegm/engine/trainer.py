...
from isegm.utils.vis import draw_probmap, draw_points, get_boundaries
...
class ISTrainer(object): 
    def __init__(self, model, cfg, model_cfg, loss_cfg,
        ...
        self.use_edgeScribbles = use_edge_scribbles
        ...
        logger.info(f'Use edge scribbles: {use_edge_scribbles}')
    ...
    def batch_forward(self, batch_data, validation=False):
        ...
        with torch.set_grad_enabled(not validation):
            ...
            gt_boundary = batch_data['boundarys']

            edges = np.zeros_like(np.empty((image.shape[0],2,image.shape[2],image.shape[3])),dtype=np.int32)

            orig_boundary = gt_boundary.cpu().numpy()
            ...
            with torch.no_grad():
                ...
                for click_indx in range(num_iters):
                    ...
                    if self.use_edgeScribbles:
                        prev_output = torch.sigmoid(eval_model(net_input, points, torch.Tensor(edges))['instances'])
                        if click_indx>0:
                            points, edges = get_next_points(prev_output, gt_mask, points, orig_boundary, click_indx + 1, edges)
                        else:
                            points = get_next_points(prev_output, gt_mask, points, orig_boundary, click_indx + 1)
                    else:
            ...
            net_input = torch.cat((image, prev_output), dim=1) if self.net.with_prev_mask else image
            if self.use_edgeScribbles:
                output = self.net(net_input, points, torch.Tensor(edges))
            else:
...
def get_next_points(pred, gt, points, gt_boundary, click_indx, edges=None, pred_thresh=0.49):
    assert click_indx > 0
    pred = pred.cpu().numpy()[:, 0, :, :] # N, H, W
    gt = gt.cpu().numpy()[:, 0, :, :] > 0.5 # N, H, W
    ...
    num_points = points.size(1) // 2 # points: torch.Size([8, 48, 3])，num_points = 24
    points = points.clone()

    for bindx in range(fn_mask.shape[0]): # batch size
        ...
        indices = np.argwhere(inner_mask) # 返回非0的数组元素的索引
        if len(indices) > 0:
            ...
            if edges is not None:
                ...
                click_label = region_label_mask[bindx, coords[0], coords[1]] # 3、点击点的标记索引i
                region_label_mask_fore = (region_label_mask[bindx] == click_label) # 4、索引i所在的错分区域

                region_label_mask_back = (region_label_mask[bindx] != click_label)*5 # 索引i所在的错分区域，前景0，背景5
                label_mask = region_label_mask_fore + region_label_mask_back # 索引i所在的错分区域，前景1，背景5
                
                if is_positive:
                    edges[bindx, 0] = edges[bindx, 0] + (gt_boundary[bindx]==label_mask)
                else:
                    edges[bindx, 1] = edges[bindx, 1] + (gt_boundary[bindx]==label_mask)
                edges = (edges > 0)
