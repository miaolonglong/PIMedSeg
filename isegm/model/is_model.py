...
class ISModel(nn.Module):
    ...
    def forward(self, image, points, edges=None):
        image, prev_mask = self.prepare_input(image)
        
        coord_features = self.get_coord_features(image, prev_mask, points, edges)

        if self.rgb_conv is not None:
            x1 = torch.cat((image, coord_features), dim=1) 
            x = self.rgb_conv(x1)
            
            
            outputs = self.backbone_forward(x)
        else:
            interactions = coord_features
            coord_features = self.maps_transform(coord_features)
            outputs = self.backbone_forward(image, interactions, coord_features)
    ...
    def get_coord_features(self, image, prev_mask, points, edges):
        if self.clicks_groups is not None:
            ...
            coord_features = self.dist_maps(image, points, edges)
