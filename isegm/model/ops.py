...
class DistMaps(nn.Module):
    ...
    def get_coord_features(self, points, edges, batchsize, rows, cols):
        if self.cpu_mode:
            coords = []
            ...
            self.cnt += 1
            num_points = points.shape[1] // 2 # 24 points
            points = points.view(-1, points.size(2)) # torch.Size([8x48, 3])
            points, points_order = torch.split(points, [2, 1], dim=1) # 切块，切成 torch.Size([8x48, 2]) 和 torch.Size([8x48, 1])

            invalid_points = torch.max(points, dim=1, keepdim=False)[0] < 0 # 返回points每行的最大值列表中小于0的bool列表
            row_array = torch.arange(start=0, end=rows, step=1, dtype=torch.float32, device=points.device)
            col_array = torch.arange(start=0, end=cols, step=1, dtype=torch.float32, device=points.device)
 
            coord_rows, coord_cols = torch.meshgrid(row_array, col_array) # 生成网格，即生成坐标
            coords = torch.stack((coord_rows, coord_cols), dim=0).unsqueeze(0).repeat(points.size(0), 1, 1, 1) # torch.Size([384, 2, 320, 480])
            
            
            add_xy = (points * self.spatial_scale).view(points.size(0), points.size(1), 1, 1)  # torch.Size([384, 2, 320, 480])
            coords.add_(-add_xy)
            ...
            coords = coords.min(dim=1)[0]  # -> (bs * num_masks * 2) x 1 x h x w ， torch.Size([16, 1, 320, 480])
            coords = coords.view(-1, 2, rows, cols) # torch.Size([8, 2, 320, 480])
        if self.use_disks:
        ...
        if edges is None:
            ...
            coords[:,:1,:,:] = coords[:,:1,:,:] + edges[:,:1,:,:]
            coords[:,1:,:,:] = coords[:,1:,:,:] + edges[:,1:,:,:]
            return coords
    def forward(self, x, coords, edges):
        return self.get_coord_features(coords, edges, x.shape[0], x.shape[2], x.shape[3])
