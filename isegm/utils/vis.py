...
def draw_with_blend_and_clicks(img, mask=None, edge=None, alpha=0.6, clicks_list=None, pos_color=(0, 255, 0),
    ...
    if mask is not None:
        ...
        if edge is not None:
            mask_rid_of_edge = (mask - edge[0].astype(np.int8) - edge[1].astype(np.int8)) > 0
        else:
            mask_rid_of_edge = mask

        rgb_mask = palette[mask_rid_of_edge.astype(np.uint8)]

        if edge is not None:
            rgb_edge_pos = palette[edge[0].astype(np.uint8)*3]
            rgb_edge_neg = palette[edge[1].astype(np.uint8)*3]

        mask_region = (mask > 0).astype(np.uint8)
        if edge is not None:
            result = result * (1 - mask_region[:, :, np.newaxis]) + \
                (1 - alpha) * mask_region[:, :, np.newaxis] * result + \
                alpha * rgb_mask + alpha * rgb_edge_pos + alpha * rgb_edge_neg
        else:
            result = result * (1 - mask_region[:, :, np.newaxis]) + \
                (1 - alpha) * mask_region[:, :, np.newaxis] * result + \
                alpha * rgb_mask
