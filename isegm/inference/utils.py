...
from isegm.data.datasets import GrabCutDataset, BerkeleyDataset, DavisDataset, SBDEvaluationDataset, PascalVocDataset, BUSIDataset
...
def get_dataset(dataset_name, cfg):
    if dataset_name == 'GrabCut':
        ...
        dataset = BUSIDataset(cfg.BUSI_PATH)
    elif dataset_name == 'BUSI1800':
        dataset = BUSIDataset(cfg.BUSI1800_PATH)
    elif dataset_name == 'LITS':
        dataset = BUSIDataset(cfg.LITS_PATH)
    elif dataset_name == 'LITS_Tumor':
        dataset = BUSIDataset(cfg.LITS_Tumor_PATH)
    elif dataset_name == 'CHAOS_CT':
        dataset = BUSIDataset(cfg.CHAOS_CT_PATH)
    elif dataset_name == 'MSD_Brain':
        dataset = BUSIDataset(cfg.MSD_Brain_PATH)
    elif dataset_name == 'BraTS_2018':
        dataset = BUSIDataset(cfg.BraTS_2018_PATH)
    elif dataset_name == 'Btcv_tcia':
        dataset = BUSIDataset(cfg.BTCV_TCIA_SPLEEN)
    else:
        dataset = None
...
def compute_noc_metric_my(all_ious, iou_thrs, max_clicks=20): 
    def _get_noc(iou_arr, iou_thr): # iou_arr 是一个 1x20 的矩阵
        vals = iou_arr >= iou_thr # 返回满足条件的索引列表
        return np.argmax(vals) + 1 if np.any(vals) else max_clicks # 返回最少点击次数

    noc_list = []
    over_max_list = [] # 达到某一IoU指标时，超过最大点击次数的图片数目列表 
    over_max_id_list = []
    for iou_thr in iou_thrs:
        scores_arr = np.array([_get_noc(iou_arr, iou_thr)
                               for iou_arr in all_ious], dtype=np.int_)
        ...
        score = scores_arr.mean()
        over_max = (scores_arr == max_clicks).sum()
        each_over_max_list = np.where(scores_arr == max_clicks)
        over_max_id_list.append(each_over_max_list[0])
        noc_list.append(score)
        over_max_list.append(over_max)

    return noc_list, over_max_list,over_max_id_list
...
def get_results_table(noc_list, over_max_list, brs_type, dataset_name, mean_spc, elapsed_time, mean_spi, offset_spi,
                      n_clicks=20, model_name=None):
    table_header = (f'|{"BRS Type":^13}|{"Dataset":^11}|'
                    f'{"NoC@80%":^9}|{"NoC@85%":^9}|{"NoC@90%":^9}|'
                    f'{">="+str(n_clicks)+"@85%":^9}|{">="+str(n_clicks)+"@90%":^9}|'
                    f'{"SPC,s":^7}|{"Time":^9}|'
                    f'{"SPI,s":^7}|{"Offset SPI,s":^13}|')
    ...
    table_row += f'{mean_spi:^7.2f}|{offset_spi:^13.2f}|'
